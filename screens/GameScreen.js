...
createExplosion(x, y) {
  ...
  setTimeout(() => {
    this.setState(prevState => ({
      explosions: prevState.explosions.filter(
        explosion => explosion.row !== x || explosion.column !== y
      ),
    }));
...
updateLevel() {
  ...
  if (level >= levelThresholds.length) {
    // Arrêt de la progression du niveau lorsque le niveau maximum est atteint
    return;
  }

  // Vérification si le score atteint le seuil du prochain niveau
  if (score >= levelThresholds[level - 1]) {
    ...
    // Navigation vers l'écran de chargement du niveau avec le score accumulé
...
stopTimer() {
  ...
  startButtonAnimation() {
    Animated.loop(
      Animated.sequence([
        Animated.timing(this.state.buttonAnimation, { // Utiliser buttonAnimation ici
          toValue: 1,
          duration: 500,
    ...
    findHintMove = () => {
        ...
        for (let col = 0; col < grid[row].length; col++) {
          ...
          for (let dRow = -1; dRow <= 1; dRow++) {
            for (let dCol = -1; dCol <= 1; dCol++) {
              ...
              if (
                row + dRow >= 0 && row + dRow < grid.length &&
                col + dCol >= 0 && col + dCol < grid[row].length
              ) {
          ...
          if (col < grid[row].length - 2) {
            ...
            if (grid[row][col] === grid[row][col + 1]) {
              ...
              if (
                (col > 1 && grid[row][col - 2] === grid[row][col]) ||
                (col < grid[row].length - 3 && grid[row][col + 3] === grid[row][col])
              ) {
                console.log(`Found hint move at row: ${row}, column: ${col}`);
                return { row, column: col };
              }
          ...
          if (row < grid.length - 2) {
            ...
            if (grid[row][col] === grid[row + 1][col]) {
              ...
              if (
                (row > 1 && grid[row - 2][col] === grid[row][col]) ||
                (row < grid.length - 3 && grid[row + 3][col] === grid[row][col])
              ) {
                console.log(`Found hint move at row: ${row}, column: ${col}`);
                return { row, column: col };
              }
      ...
      let grid3 = [ /* une grille avec un mouvement d'aide possible */ ];
        console.log(findHintMove(grid3));  // devrait afficher le mouvement d'aide

        let grid4 = [ /* une grille sans mouvement d'aide possible */ ];
  ...
  highlightHelpCell = () => {
    ...
    if (hintMove) {
      ...
      Alert.alert(
        "Aide",
        "Désolé, aucun mouvement aidant à aligner trois diamants n'a été trouvé.",
        [
          {text: 'OK'}
        ],
        { cancelable: false }
    ...
    <View style={styles.row}>
      {row.map((cell, column) => {
        ...
        if (ishighlightHelpCell) {
          Animated.loop(
            Animated.sequence([
              Animated.timing(blinkValue, { toValue: 0.2, duration: 500, useNativeDriver: false }),
              Animated.timing(blinkValue, { toValue: 1, duration: 500, useNativeDriver: false })
            ])
          ).start();
        }
  ...
  <View style={[styles.grid, paused ? { opacity: 0 } : {}]}>
    {grid.map((row, rowIndex) => (
      ...
      const pauseTextComponent = (
        <Animatable.Text
          style={{ fontSize: 100, color: 'yellow', textShadowColor: 'black', textShadowRadius: 10 }}
          animation="flash"
          iterationCount="infinite"
        >Pause</Animatable.Text>
      );
        ...
        >
          <View style={styles.container}>
            ...
            <View style={styles.buttonContainer}>
              <TouchableOpacity
                onPress={() => this.handleGameCompletion(username)}
                style={[styles.scoreButton, { borderColor: buttonBorderColor, backgroundColor: '#841584' }]}
              >
                <Text style={[styles.buttonText, { color: '#ffffff' }]}>Fin de partie</Text>
              </TouchableOpacity>

              <TouchableOpacity
                onPress={this.handlePauseResumeGame}
                style={[styles.scoreButton, { borderColor: buttonBorderColor, backgroundColor: '#841584' }]}
              >
              ...
              <TouchableOpacity
                onPress={this.highlightHelpCell}
                style={[styles.scoreButton, { borderColor: buttonBorderColor, backgroundColor: '#841584' }]}
              >
